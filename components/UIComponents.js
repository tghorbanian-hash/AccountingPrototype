...
export const Card = ({ children, className = '', noPadding = false, isSurface = false }) => {
  const baseClass = isSurface 
    ? `${THEME.colors.surface} border border-white/20`
    : `${THEME.colors.glassBg} ${THEME.colors.glassBlur} border ${THEME.colors.glassBorder} ${THEME.shadows.card}`;
  
  return (
    <div className={`${baseClass} ${THEME.radius.card} ${noPadding ? '' : 'p-6'} ${className}`}>
      {children}
    </div>
  );
};

export const Button = ({ children, variant = 'primary', icon: Icon, isLoading, className = '', onClick, type = 'button', disabled }) => {
  ...
  return (
    <button type={type} onClick={onClick} disabled={isLoading || disabled} className={`${baseStyle} ${variants[variant]} ${className}`}>
...
export const IconButton = ({ icon: Icon, onClick, color = 'slate', title }) => {
  ...
  return (
    <button onClick={onClick} title={title} className={`p-2 rounded-lg transition-colors ${colors[color] || colors.slate}`}>
      <Icon size={16} />
    </button>
  );
};
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div className="relative group">
        ...
        <input {...props} className={`w-full bg-white/50 border border-slate-200/60 ${THEME.radius.field} py-3 ${Icon ? (isRtl ? 'pr-11 pl-4' : 'pl-11 pr-4') : 'px-4'} outline-none focus:border-${THEME.colors.primary}-500 focus:ring-4 focus:ring-${THEME.colors.primary}-500/10 focus:bg-white transition-all text-xs font-bold text-slate-700 placeholder:text-slate-400`} />
      </div>
    </div>
  );
};
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div className="relative">
        <select {...props} className={`w-full bg-white/50 border border-slate-200/60 ${THEME.radius.field} py-3 px-4 appearance-none outline-none focus:border-${THEME.colors.primary}-500 focus:ring-4 focus:ring-${THEME.colors.primary}-500/10 focus:bg-white transition-all text-xs font-bold text-slate-700 cursor-pointer`}>
          {children}
        </select>
...
export const Toggle = ({ checked, onChange, labelActive, labelInactive }) => {
  return (
    <div className="flex items-center gap-3 cursor-pointer" onClick={() => onChange(!checked)}>
      <div className={`w-10 h-6 rounded-full p-1 transition-colors duration-300 relative ${checked ? `bg-${THEME.colors.primary}-500` : 'bg-slate-300'}`}>
        <div className={`w-4 h-4 bg-white rounded-full shadow-md transition-transform duration-300 ${checked ? (document.dir === 'rtl' ? '-translate-x-4' : 'translate-x-4') : 'translate-x-0'}`}></div>
      </div>
...
window.UI = { Card, Button, IconButton, InputField, SelectField, Toggle, Badge, THEME };
