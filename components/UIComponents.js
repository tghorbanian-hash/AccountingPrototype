...
export const Button = ({ 
  ...
  return (
    <button onClick={onClick} disabled={isLoading || disabled} className={`${baseStyle} ${variants[variant]} ${sizes[size]} ${className}`}>
...
export const LOV = ({ label, placeholder, onSearch, isRtl, className = '' }) => {
  return (
    <div className={`w-full ${className}`}>
       {label && <label className="block text-[11px] font-bold text-slate-700 mb-1">{label}</label>}
       <div className="flex">
          <input placeholder={placeholder} className={`flex-1 ${THEME.colors.surface} border ${THEME.colors.border} ${isRtl ? 'rounded-r border-l-0' : 'rounded-l border-r-0'} ${THEME.metrics.inputHeight} px-2 outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 text-[12px]`}/>
          <button className={`bg-slate-100 border ${THEME.colors.border} px-3 hover:bg-slate-200 text-slate-600 ${isRtl ? 'rounded-l border-r-0' : 'rounded-r border-l-0'}`}><List size={14} /></button>
       </div>
    </div>
  );
};
...
export const SelectField = ({ label, children, isRtl, className = '', ...props }) => (
  <div className={`w-full ${className}`}>
    ...
    <div className="relative">
      <select {...props} className={`w-full ${THEME.colors.surface} border ${THEME.colors.border} ${THEME.metrics.radius} ${THEME.metrics.inputHeight} pl-2 pr-8 appearance-none outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 text-[12px] text-slate-900 cursor-pointer`}>
        {children}
      </select>
      <div className={`absolute inset-y-0 ${isRtl ? 'left-2' : 'right-2'} flex items-center pointer-events-none text-slate-500`}><ChevronDown size={14} /></div>
    </div>
  </div>
);
...
export const Toggle = ({ checked, onChange, label }) => (
  <div className="flex items-center gap-2 cursor-pointer select-none group" onClick={() => onChange(!checked)}>
    <div className={`w-8 h-4 rounded-full p-0.5 transition-colors duration-200 relative ${checked ? 'bg-indigo-600' : 'bg-slate-300 group-hover:bg-slate-400'}`}>
      <div className={`w-3 h-3 bg-white rounded-full shadow-sm transition-transform duration-200 ${checked ? (document.dir === 'rtl' ? '-translate-x-4' : 'translate-x-4') : 'translate-x-0'}`}></div>
    </div>
...
export const Badge = ({ children, variant = 'neutral' }) => {
  ...
  return <span className={`px-2 py-0.5 rounded text-[11px] font-semibold border ${styles[variant] || styles.neutral}`}>{children}</span>;
};

// --- 2. COMPLEX COMPONENTS ---

export const DataGrid = ({ columns, data = [], actions, onSelectAll, onSelectRow, selectedIds = [], isLoading, isRtl, pagination }) => {
  return (
    <div className="flex flex-col h-full border border-slate-300 rounded bg-white shadow-sm overflow-hidden">
      <div className="bg-slate-50 border-b border-slate-200 p-2 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2"><Button variant="ghost" size="sm" icon={Settings} /></div>
      </div>
      <div className="flex-1 overflow-auto relative">
        <table className="w-full border-collapse text-[12px]">
          <thead className="bg-slate-100 sticky top-0 z-10 shadow-sm">
            <tr>
              <th className="w-10 px-2 py-2 border-b border-slate-300 text-center"><input type="checkbox" className="rounded border-slate-400" checked={data.length > 0 && selectedIds.length === data.length} onChange={(e) => onSelectAll && onSelectAll(e.target.checked)}/></th>
              {columns.map((col, idx) => (
                <th key={idx} className={`px-3 py-2 border-b border-slate-300 font-bold text-slate-700 uppercase tracking-tight text-start whitespace-nowrap ${col.width || ''}`}>
                  <div className="flex items-center gap-1 cursor-pointer hover:text-indigo-700">{col.header}{col.sortable && <ChevronDown size={12} className="opacity-50" />}</div>
                </th>
              ))}
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {isLoading ? (<tr><td colSpan={100} className="p-8 text-center text-slate-500"><Loader2 className="animate-spin mx-auto mb-2"/>Loading Data...</td></tr>) : data.map((row, rowIndex) => {
              const isSelected = selectedIds.includes(row.id);
              return (
                <tr key={row.id || rowIndex} className={`transition-colors ${isSelected ? THEME.colors.rowSelected : 'hover:bg-slate-50'} ${rowIndex % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                  <td className="px-2 py-1.5 border-r border-slate-100 text-center"><input type="checkbox" className="rounded border-slate-300" checked={isSelected} onChange={(e) => onSelectRow && onSelectRow(row.id, e.target.checked)}/></td>
                  {columns.map((col, cIdx) => (<td key={cIdx} className="px-3 py-1.5 border-r border-slate-100 text-slate-700 truncate max-w-xs">{col.render ? col.render(row) : row[col.field]}</td>))}
                  {actions && (<td className="px-2 py-1.5 text-center"><div className="flex items-center justify-center gap-1">{actions(row)}</div></td>)}
                </tr>
              );
            })}
          </tbody>
      ...
      <div key={item.id} className="relative">
        ...
        <div 
          ...
          className={`
            ...
            ${isActive && !hasChildren ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}
            ${depth === 0 ? 'mx-4' : (isRtl ? `mr-8 ml-2` : `ml-8 mr-2`)}
          `}
        >
          <div className="shrink-0 flex items-center justify-center w-5 h-5">
             {hasChildren ? (
               <ChevronDown size={14} className={`text-slate-400 transition-transform duration-200 ${isExpanded ? '' : (isRtl ? 'rotate-90' : '-rotate-90')}`} />
             ) : (
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-2 shrink-0">
        <div className="relative">
          <input 
            ...
            className={`
              w-full bg-slate-100 border-none rounded-lg text-xs h-8 focus:ring-2 focus:ring-indigo-100 outline-none
              ${isRtl ? 'pr-8 pl-2' : 'pl-8 pr-2'} transition-all
            `}
          />
          <Search size={14} className={`absolute top-1/2 -translate-y-1/2 text-slate-400 ${isRtl ? 'right-2.5' : 'left-2.5'}`} />
          {searchTerm && (
            <button onClick={() => setSearchTerm('')} className={`absolute top-1/2 -translate-y-1/2 text-slate-400 hover:text-red-500 ${isRtl ? 'left-2' : 'right-2'}`}>
              <X size={12} />
            </button>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar py-2">
        {visibleItems.length > 0 ? (
          ...
          <div className="text-center text-slate-400 text-xs mt-4 py-4 italic">
            {isRtl ? "موردی یافت نشد" : "No items found"}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className={`bg-white rounded shadow-2xl flex flex-col w-full ${sizes[size]} max-h-[90vh] animate-in zoom-in-95`}>
        ...
        {footer && (<div className="px-4 py-3 border-t border-slate-200 bg-slate-50 flex items-center justify-end gap-2 shrink-0 rounded-b">{footer}</div>)}
      </div>
    </div>
  );
};
...
export const DatePicker = ({ label, isRtl, className = '', ...props }) => (
  <div className={`w-full ${className}`}>
    {label && <label className="block text-[11px] font-bold text-slate-700 mb-1">{label}</label>}
    <div className="relative">
       <input type="date" {...props} className={`w-full ${THEME.colors.surface} border ${THEME.colors.border} ${THEME.metrics.radius} ${THEME.metrics.inputHeight} px-2 outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 text-[12px] text-slate-900 uppercase font-mono`} />
    </div>
  </div>
);

// --- Exports ---
window.UI = { Button, InputField, SelectField, Toggle, Badge, DataGrid, TreeMenu, Modal, DatePicker, LOV, THEME };
